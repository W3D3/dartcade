<script lang="ts">
  ...
  const firstTo      = $derived(game.firstTo as number | undefined)
  const hasLegs      = $derived(firstTo !== undefined)
...
<section
  ...
  class="flex flex-col gap-0 box-border p-7 rounded-[18px] h-full
         {isActive
  ...
  <div class="flex justify-between items-center">
    <div class="flex items-center gap-3">
      ...
      <span class="text-[20px] font-semibold {isActive ? 'text-text' : 'text-[#8a8e83]'}">
        {player.name}
      </span>
    </div>
    {#if hasLegs}
  ...
  <div class="mt-3">
    {#if isWinner}
      <span class="inline-flex items-center h-7 px-3 rounded-full bg-accent text-accent-fg
                   text-[12px] font-bold tracking-[0.08em] uppercase">Winner!</span>
    {:else if isActive}
      <span class="inline-flex items-center h-7 px-3 rounded-full bg-accent text-accent-fg
                   text-[12px] font-bold tracking-[0.08em] uppercase">Throwing</span>
    {:else}
      <span class="inline-flex items-center h-7 px-3 rounded-full border border-line-2
                   text-[#5a5e55] text-[12px] font-semibold tracking-[0.08em] uppercase">Up next</span>
    {/if}
  </div>
  ...
  {:else}
    ...
    <div class="flex flex-col gap-[6px] p-[16px_18px] rounded-[12px] mt-4
    ...
    <div class="flex gap-7 text-[15px] mt-4 {isActive ? 'text-text-muted' : 'text-text-dim'}">
